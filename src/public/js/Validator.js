...
Validator.isUser = function(selector){
    return {
        selector,
        test: function(value){
            return value.trim() ? undefined : 'Vui lòng nhập tên đăng nhập này';
        }
    }
}
...
Validator.isPhone = function(selector){
    return {
        selector,
        test: function(value){
            return value.length === 10 ? undefined : 'Phải nhập đúng số điện thoại';
        }
    }
}
...
Validator.isPassWordNL = function(selector, getConfirmValue){
    return {
        selector,
        test: function(value){
            return value === getConfirmValue() ? undefined : 'Không trùng với mật khẩu';
        }
    }
}
